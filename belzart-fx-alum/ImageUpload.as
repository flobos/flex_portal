    ...
    //        fr.addEventListener(Event.OPEN, iniciarSubida);
     ...
     //       fr.addEventListener(HTTPStatusEvent.HTTP_STATUS, httpStatus);
		...
		public function archivoSeleccionado(event:Event):void {
			...
			var p:String;
    		if (fr.type == ".doc" || fr.type == ".docx" || fr.type == ".xls" || fr.type == ".xlsx" || fr.type == ".pdf") {
			...
			{
				mx.controls.Alert.show('El archivo debe ser Word(.doc,.docx), Pdf(.pdf) o Excel(.xls,.xlsx)', 'Ingreso')
				btn.enabled = false;
			}
