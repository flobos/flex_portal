...
{
	this.dg_preguntas.dataProvider = event.result;
}
//***********************************************************************************************************
...
//	mx.controls.Alert.show(this.dg_cursos.selectedItem.campo_dos);
	if (this.dg_pruebas.selectedIndex < 0) {
		mx.controls.Alert.show("Debe seleccionar una prueba para efectuar la descarga","Ayuda");
	}
	else
	{
		if(this.dg_pruebas.selectedItem.campo_dos != null){
			var request:URLRequest = new URLRequest(this.dg_pruebas.selectedItem.campo_dos);
			var archivo_prueba:FileReference = new FileReference();
			archivo_prueba.download(request);
		}
		else{
			mx.controls.Alert.show("No existe un archivo asociado a esta prueba","Ayuda");
		}
	}
}
//***********************************************************************************************************
...
//	mx.controls.Alert.show(this.dg_cursos.selectedItem.campo_dos);
	...
	else
	{
		...
		ro_busca_datos_alumnos.muestra_preguntas_respuestas(this.lbl_bus_trabajo.text);
		limpiar_matriculas();
		limpiar_cursos();
		limpiar_ficha();
	}
...
//***********************************************************************************************************
private function pruebas_click( e:ListEvent ):void {
	ro_busca_datos_alumnos.muestra_preguntas_respuestas(this.dg_pruebas.selectedItem.campo_tres);
...
//***********************************************************************************************************
private function btn_recalcula_nota_click(eventObj:CloseEvent):void {
	if (this.dg_pruebas.selectedIndex  >= 0 && this.dg_pruebas.selectedItem.campo_seis == 'Selección múltiple en tiempo real'){
		if (eventObj.detail==Alert.OK) {
			ro_busca_datos_alumnos.recalcular_nota(this.dg_pruebas.selectedItem.campo_tres);
            mx.controls.Alert.show("La Nota del Trabajo "+this.dg_pruebas.selectedItem.campo_tres+ " fué recalculada", "Alerta");
   		}
 	}
	else {
		mx.controls.Alert.show("Debe seleccionar una Prueba Objetiva para recalcular la nota", "Alerta");
	}
...
//	mx.controls.Alert.show(this.dg_cursos.selectedItem.campo_dos);
	...
	this.dg_preguntas.dataProvider = undefined;
}
